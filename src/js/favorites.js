...
loadHeaderFooter().then(() => {
    ...
    recipeData.forEach((recipe) => {
      ...
      removeButton.addEventListener("click", (e) => {
        ...
        const informationContainer =
          removeButton.parentElement.previousElementSibling;
        ...
        let newCurrentLocal = currentLocal.filter(
          (r) => r.name !== nameOfRecipe.textContent,
        );
        ...
        heroContainerMain.querySelector(".heroText p").innerHTML =
          `<span>${newCurrentLocal.length !== 0 ? newCurrentLocal.length : 0}</span> Recipes saved with love and hunger`;
      });
  ...
  heroContainerMain.querySelector(".heroText").innerHTML =
    ...
    <p><span>${savedNumber ? savedNumber.length : 0}</span> Recipes saved with love and hunger</p>`;
